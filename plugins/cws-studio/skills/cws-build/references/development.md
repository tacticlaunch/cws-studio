...
> Source: **not in source transcripts** — the playbook predates the
> Chrome 114 Side Panel API (released May 2023, after Module II was
> recorded). The six-type table above is the canonical playbook list. This
...
using it.** The playbook's core rule (match the SERP, copy the dominant
...
- Whether `sidePanel` permission triggers the same review scrutiny as
  `<all_urls>` host permissions is not documented in the playbook.
- Library/script bundling rules likely follow the local-page-in-tab
  pattern (everything local, no remote scripts), but the playbook does
